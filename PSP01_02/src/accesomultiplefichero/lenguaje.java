      ...
      if (osName.toUpperCase().contains("WIN")){ //Windows
        if (args.length > 1)
          ...
          //Hemos recibido la ruta del fichero en la línea de comandos
               ...
               nombreFichero = "/home/lenguaje.txt";
               //Fichero que se utilizará por defecto
        }
      }
      //Preparamos el acceso al fichero
      archivo = new File(nombreFichero);
      for (int i=0; i<orden; i++)//aumentamos las situaciones de concurrencia
         try{
            ...
            String palabra="";
            //Generamos una palabra de 5 letras
            for(int j = 0; j < 5; j ++) 
        		palabra += (char)(97 + (Math.random() * 25)); //Cogemos aleatoriamente una letra de la a - z
            // Lectura del fichero
            raf.seek(raf.length()); //volvemos a colocarnos al principio del fichero
            raf.writeUTF(palabra + "\r\n"); //escribimos el valor
